...
ngx_http_init_connection(ngx_connection_t *c)
{
    ...
    c->write->handler = ngx_http_empty_handler; //ngx_http_empty_handler 该连接结构的写事件
...
ngx_http_process_request_line(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_AGAIN) {
            n = ngx_http_read_request_header(r);// 正常读取到数据
        ...
        rc = ngx_http_parse_request_line(r, r->header_in); // 读取完成正常解析
        ...
        if (rc == NGX_OK) {
            ...
            if (r->http_version < NGX_HTTP_VERSION_10) {
                ...
                ngx_http_process_request(r); //真正的进入处理请求的部分
                return;
            }
            ...
            ngx_http_process_request_headers(rev);//循环的读取所有的请求头
