...
ngx_http_upstream_create(ngx_http_request_t *r) 
{
    ...
    u = ngx_pcalloc(r->pool, sizeof(ngx_http_upstream_t)); // 创建nginx upstream 的
    if (u == NULL) {
        return NGX_ERROR;
    }
    ...
    u->headers_in.content_length_n = -1;  // 设置header 相关信息
    u->headers_in.last_modified_time = -1;// 设置访问时间
...
ngx_http_upstream(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    ngx_http_upstream_srv_conf_t  *uscf; // upstream 的结构体
