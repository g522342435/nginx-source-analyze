 ...
 */
    ...
    if (sigprocmask(SIG_BLOCK, &set, NULL) == -1) { //sigprocmask()函数只为单线程的进程定义的，在多线程中要使用pthread_sigmask变量
    ...
    ngx_setproctitle(title); // 设置进程的名称
   ...
   //可以获得每个模块的配置结构 这个结构
    ...
    ngx_start_worker_processes(cycle, ccf->worker_processes,
                               NGX_PROCESS_RESPAWN);//respawn标记是挂了要不要重启
    ngx_start_cache_manager_processes(cycle, 0);//   //Cache管理进程与cache加载进程的主流程
...
ngx_single_process_cycle(ngx_cycle_t *cycle)
{
   ...
   // 使用 UNIX域套接字 socketpair() 异步通讯机制: 
   //nginx在创建worker前将先调用 socketpair(int channel[2]) 然后将 
   //channel[0-1]设置为异步通知方式,并注册evnet事件,
   //父进程使用channel[0],子进程使用channel[1]实现双方的通讯
static void
ngx_start_worker_processes(ngx_cycle_t *cycle, ngx_int_t n, ngx_int_t type)
{
    ...
    for (i = 0; i < n; i++) {
	  ...
	  /*在ngx_spawn_process创建好一个worker进程返回后，
	  master进程就将worker进程的pid、worker进程在ngx_processes数组中的位置及
	  channel[0]传递给前面已经创建好的worker进程，
	  然后继续循环开始创建下一个worker进程。
	  刚提到一个channel[0]，这里简单说明一下：
	  channel就是一个能够存储2个整型元素的数组而已，
	  这个channel数组就是用于socketpair函数创建一个进程间通道之用的。
	  master和worker进程以及worker进程之间都可以通过这样的一个通道进行通信，
	  这个通道就是在ngx_spawn_process函数中fork之前调用socketpair创建的。*/  
...
//http://blog.csdn.net/hpghy123456/article/details/8751165
static void
...
ngx_worker_process_init(ngx_cycle_t *cycle, ngx_int_t worker)
{
    ...
    if (worker >= 0 && ccf->priority != 0) {
        if (setpriority(PRIO_PROCESS, 0, ccf->priority) == -1) { // 设置进程优先执行权限
            ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno,
    ...
    for (i = 0; ngx_modules[i]; i++) {
        if (ngx_modules[i]->init_process) { //ngx_event_process_init
            if (ngx_modules[i]->init_process(cycle) == NGX_ERROR) {
                /* fatal */
                exit(2);
            }
...
//ngx_add_channel_event把句柄ngx_channel(当前worker的channel[1])上建立的连接
//的可读事件加入事件监控队列，事件处理函数为
//ngx_channel_hanlder(ngx_event_t *ev)。当有可读事件的时候，
//ngx_channel_handler负责处理消息，具体代码可以查看
//src/os/unix/ngx_channel.c,过程如下
static void
