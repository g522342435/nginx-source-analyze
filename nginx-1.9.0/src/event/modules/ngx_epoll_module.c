...
static struct epoll_event  *event_list; //链表   事件
...
failed:
    ...
    ngx_file_aio = 0;
       ...
       // 1、创建全局epoll句柄、event_list
        //2、设置ngx_event_actions = ngx_epoll_module_ctx.actions;以后worker进程都是使用ngx_event_actions 全局变量来进行事件处理。
...
ngx_epoll_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    c = ev->data;//每个事件的data成员存放着其对应的ngx_connection_t连接  
