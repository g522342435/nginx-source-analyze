...
/*
跟http module 一样 ngx_http_block
...
ngx_module_t  ngx_event_core_module = {
    ...
    ngx_event_process_init,                /* init process */  // 每个子进程会调用 
    NULL,                                  /* init thread */
    NULL,                                  /* exit thread */
    NULL,                                  /* exit process */
    NULL,                                  /* exit master */
...
//调用ngx_trylock_accept_mutex()试着给cycle上锁，并把ngx_accept_mutex_held设为1  
    ...
    */
    ...
    if (ngx_accept_mutex_held) {
     ...
     /* 
    delta是之前统计的耗时，存在毫秒级的耗时，
    //就对所有时间的timer进行检查， 
    如果timeout 就从time rbtree中删除到期的timer，
    //同时调用相应事件的handler函数处理 
    */ 
    if (delta) {
...
ngx_event_process_init(ngx_cycle_t *cycle)
{
    ...
    if (ccf->master && ccf->worker_processes > 1 && ecf->accept_mutex) {
        ...
        ngx_use_accept_mutex = 0; // 不使用錫ccept_mutex
    }
    ...
    for (m = 0; ngx_modules[m]; m++) {
        ...
        //actions.init  调用ngx_epoll_init 完成 epoll 的初始化
    ...
    //分配connection_n个空间给read_events和write_events,初始化这些
    //read_events和write_events 
    ...
    rev = cycle->read_events; //指向读事件的event
    ...
    for (i = 0; i < cycle->connection_n; i++) {
     ...
     //把connection和read_event，write_event联系起来，
     //每个connection都指向一个read_event和write_event
